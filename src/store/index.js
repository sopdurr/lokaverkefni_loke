...
export default new Vuex.Store({
  state: {
    ...
    user: undefined,
    assignments: [
      { name: 'Interrogate the Kool-Aid figure from Family guy', employee: 'siggilögga', closed: true },
      { name: 'Pickup trash in front of the police station', employee: 'siggilögga', closed: false },
    ],
    criminals: [
      {
        crimeId: 1,
    ...
    victims: [
      { crimeId: 1, name: 'Gaui Litli', homeAddress: 'Litli Bær', phoneNumber: 5412000, age: 68 },
      { crimeId: 2, name: 'Harry Potter', homeAddress: 'Hogwarts', phoneNumber: 9992000, age: 20 }
    ],
    severity: [
      { number: 1 },
      { number: 2 },
    ...
    crimes: [
      {
        ...
        date: '2019-01-25',
        description: 'Yelled at Gaui litli for 10 minutes straight',
        severity: 1,
        notes: undefined,
        closed: true,
      },
      {
        ...
        date: '2021-06-27',
        description: 'Hacked himself into Gringotts bank and stole all of Harry Potter\'s money',
        severity: 8,
        notes: undefined,
        closed: false,
      },
  ...
  mutations: {
    ...
    addVictim(state, payload) {
      if (payload.name != null || payload.name != undefined &
        ...
        payload.crimeId = state.victims.length + 1;
        state.victims.push(payload)
      }
    },
    addSuspects(state, payload) {
      if (payload.name != null || payload.name != undefined) {
        payload.crimeId = state.criminals.length + 1;
    ...
    addCrime(state, payload) {
      if (payload.crimePlace != null || payload.crimePlace != undefined) {
        payload.crimeId = state.crimes.length + 1;
